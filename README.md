...
4. In Grafana go to `Alert rules` > `New alert rule`. 
    ...
    - Under queries select `Code` and paste the following query: `sum by (ip, message)(count_over_time({job="modsec"} |~ "ModSecurity: (Warning|Emergency|Alert|Critical)" | pattern "<_> [<_> <_>] [<_> <_>] [<_> <_>] [<_> <ip>] <message> [d]" [5m]))`.
